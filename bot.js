...
async function updateBeer(user_id, db){
    try{
        var user = await db.collection("people").findAndModify({'groupme_user_id': user_id}, {$inc: { "beer_count": 1 }}, {new: true})
...
async function resetBeer(user_id, db){
    try{
        var user = await db.collection("people").findAndModify({'groupme_user_id': user_id}, {$set: { "beer_count": 0 }}, {new: true})
...
async function resetBeerTimeAndIncBeer(user_id, db){
    try{
        ...
        var user = await db.collection("people").findAndModify({'groupme_user_id': user_id}, {$set: { "beer_time": hours_epoch, "beer_count": 1}}, {new: true})
